...
TEST(TINY_FD_NRM, ABM_RecieveTwoConsequentIFrames)
{
    establishConnection();
...
TEST(TINY_FD_NRM, ABM_RecieveOutOfOrderIFrames)
{
    establishConnection();
    // Now we can send I-frames
...
TEST(TINY_FD_NRM, ABM_SendSABMOnIFrameIfDisconnected)
{
...
TEST(TINY_FD_NRM, ABM_RunRxAPIVerification)
{
    // For command requests CR bit must be set, that is why address is 0x03
...
TEST(TINY_FD_NRM, ABM_RunTxAPIVerification)
{
    // For command requests CR bit must be set, that is why address is 0x03
...
TEST(TINY_FD_NRM, ABM_CheckMtuAPI)
{
    // Check MTU API
...
TEST(TINY_FD_NRM, ABM_CheckLoggerFunction)
{
    int counter = 0;
...
TEST(TINY_FD_NRM, ABM_CheckMtuAndSendSplit)
{
...
TEST(TINY_FD_NRM, ABM_CheckReceiveReadyWithCommandBitSet)
{
...
TEST(TINY_FD_NRM, ABM_CheckReceiveReadyWithCommandBitCleared)
{
    // On Receive Ready with command bit set, and if there is no data to send,
    // we should send RR frame with command bit cleared
    establishConnection(); // Establish connection first
