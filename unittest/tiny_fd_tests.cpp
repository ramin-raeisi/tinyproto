...
TEST(TINY_FD, ConnectionTestSabm)
{
    auto read_result = tiny_fd_on_rx_data(handle, (uint8_t *)"\x7E\x03\x2F\x7E", 7); // SABM frame
    ...
    CHECK_EQUAL(0x01, outBuffer[1]); // SABM control field
    CHECK_EQUAL(0x73, outBuffer[2]); // Address field
